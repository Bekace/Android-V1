...
class MainActivity : FragmentActivity() {
    ...
    private fun updateUiForState(state: PlaybackState) {
        // Reset visibility for main containers to ensure a clean slate
        binding.statusTextview.visibility = View.GONE
        binding.fragmentContainer.visibility = View.GONE

        val transaction = supportFragmentManager.beginTransaction()
        
        // Explicitly hide WebPairingFragment if it's not the current state
        webPairingFragment?.let { fragment ->
            if (fragment.isVisible && state !is PlaybackState.NeedsPairing) {
        ...
        when (state) {
            ...
            is PlaybackState.Idle -> {
                // Both containers are already GONE from initial setting
            }
        }
    ...
    private fun displayContentFragment(state: PlaybackState, transaction: androidx.fragment.app.FragmentTransaction) {
        val newFragment: Fragment? = when (state) {
            ...
            is PlaybackState.Preparing -> { // Ensure content fragments are correctly replaced/shown even in preparing state
